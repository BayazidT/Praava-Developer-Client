...
const columns = [
  ...
  },
  {
    ...
    width: 120,
    renderCell: (params) => (
      <Link className='link' to={`user/${params.id}`}>Edit</Link>
    )
...
const User = () => {
  ...
  const { id } = useParams();
  const user = rows.filter((i) => i.id == id);

  const fetchUsers = () =>{

    const requestOptions = {
      method: 'GET',
      headers: { 'Content-Type': 'application/json', token:'stingtoken' },   
  };
  APICall(requestOptions);
  }
  async function APICall(request){
    try {
      const response =await fetch(`http://127.0.0.1:5000/auth/register`, request);
